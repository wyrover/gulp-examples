  ...
  return through.obj((file, encoding, callback) => {
    ...
    if (file.isBuffer()) {
      if (action === 'encrypt') {
        ...
        let compressBuf = zlib.deflateRawSync(file.contents)
        ...
        file.contents = Buffer.concat([encrypted_key_iv, encrypted_output])
        return callback(null, file)
      } else if (action === 'decrypt') {
        const encrypted_content_len = file.contents.length - 512

        console.log(encrypted_content_len)

        let encrypted_key_iv = file.contents.slice(0, 512)
        let encrypted_content = file.contents.slice(512)
        ...
        file.contents = zlib.inflateRawSync(decrypted_content)
        return callback(null, file)        
      }
    }
